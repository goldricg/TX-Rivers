...
#plot percent change with shift from 25 deg to 10 deg
plot <- ggplot(data=z, aes(x=factor(NEAR), y=pchange, color=variable))+
  geom_point(size=4)+
  ylim(-100,100)+
  labs(x="Zone", y="Percent change", color="Mineral")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
print(plot)

plot <- ggplot(data=z, aes(pchange))+
  geom_histogram()
print(plot)

plot(ecdf(z[,"pchange"]), main = "Cumulative distribution, % change")


x <- z[which(z$pchange > 100000),]